...
class StrategyRecomendacion {
  ...
  _calcularCargaMateria(materia) {
    ...
    return {
      horasCursada,
      horasEstudio,
      horasTotales: horasCursada + horasEstudio
    };
  }
  ...
  _obtenerSituacionLaboral(estudiante) {
    ...
    if (horasLaborales > 0 && horasLaborales <= 25) return 'PASANTE_4HS';
    if (horasLaborales >= 35) return 'RELACION_DEPENDENCIA_FULLTIME';
  ...
  _limiteEstudioDisponible(disponibilidad, estudiante, factorObjetivo = 1) {
    ...
    // El factor laboral da margen de seguridad sobre horas libres de estudio.
    // No descuenta horas de cursada porque la disponibilidad ya representa tiempo fuera de clase.
  ...
  _resultado(recomendadas, noRecomendadas) {
    return {
      recomendadas,
      noRecomendadas,
      // Este valor representa horas de estudio fuera de cursada.
