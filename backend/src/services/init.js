...
async function initDatabase() {
  ...
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS estudiante (
      ...
      regularizadas_habilitan INTEGER NOT NULL DEFAULT 0,
  ...
  if (!colNames.includes('situacion_laboral')) {
    await db.exec(`ALTER TABLE estudiante ADD COLUMN situacion_laboral TEXT DEFAULT NULL`);
    console.log('[DB] Migración: columna situacion_laboral agregada.');
  }
  if (!colNames.includes('regularizadas_habilitan')) {
    await db.exec(`ALTER TABLE estudiante ADD COLUMN regularizadas_habilitan INTEGER NOT NULL DEFAULT 0`);
    console.log('[DB] Migración: columna regularizadas_habilitan agregada.');
  }
